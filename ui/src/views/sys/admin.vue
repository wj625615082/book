...
<!--      </el-table-column>-->
      <el-table-column align="center" label="操作" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button v-permission="['sys:admin']" type="primary" size="mini" @click="handleRole(scope.row)">角色</el-button>
...
<!--        <el-form-item label="管理员头像" prop="avatar">
    ...
    <el-dialog title="角色分配" :visible.sync="roleDialogVisible" width="30%">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="confirmRoleAssignment">确定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
import { getRoles, getRolesByAdmin, updateRoleByAdmin } from '@/api/role'
...
export default {
  ...
  methods: {
    ...
    handleRole(row) {
      ...
      this.fetchRoles(this.selectedAdminId)
      this.roleDialogVisible = true
    },
    fetchRoles() {
      getRolesByAdmin({ adminId: this.selectedAdminId }).then(response => {
        this.roles = response.data.list;

        // 设置 selectedRoles 根据 role 字段
    ...
    confirmRoleAssignment() {
      // 构造请求体
      const updateRoleByAdminParam = {
        adminId: this.selectedAdminId,
        roles: this.selectedRoles
      };

      // 调用 updateRoleByAdmin API
      updateRoleByAdmin(updateRoleByAdminParam).then(response => {
        this.$notify.success({
          title: '成功',
          message: '角色分配更新成功'
        });
        ...
        this.$notify.error({
          title: '错误',
          message: '角色分配更新失败: ' + error.message
        });
      });
    }
  }
